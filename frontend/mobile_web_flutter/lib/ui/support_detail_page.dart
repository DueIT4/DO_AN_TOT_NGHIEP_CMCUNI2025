...
class _SupportDetailPageState extends State<SupportDetailPage> {
  late Future<(bool, List<dynamic>, String)> _messagesFuture;
  ...
  void _loadMessages() {
    ...
    setState(() {
      _messagesFuture = ticketId == null
          ? Future.value((false, <dynamic>[], 'Không tìm thấy ticket id'))
  ...
  Future<void> _sendMessage() async {
    ...
    if (ticketId == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Không tìm thấy ticket id')),
      );
      return;
    }

    setState(() => _isSending = true);
    final (success, _, error) = await SupportService.createMessage(
      ticketId,
      message,
      file: _selectedFile,
    );
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
          Expanded(
            child: FutureBuilder<(bool, List<dynamic>, String)>(
              ...
              builder: (context, snapshot) {
                ...
                final (success, messages, error) = snapshot.data!;
                ...
                final normalizedMessages = messages
                    .map((m) => {
                          'content': m['message'] ?? '',
                          'is_user': _asInt(m['sender_id']) == _ownerUserId,
